...
# =====================================================================
    ...
    st.title("📊 Distribution & Comportement des Capteurs")
    st.markdown("**Objectif :** Identifier les **plages de valeurs critiques** où les pannes surviennent. "
                "Plus les distributions Sain/Panne sont séparées, plus la variable est discriminante.")
    ...
    col_c.metric("📗 Médiane (Sain)", f"{stats_ok['50%']:.2f}")
    col_d.metric("📕 Médiane (Panne)", f"{stats_ko['50%']:.2f}",
                 f"{stats_ko['50%']-stats_ok['50%']:+.2f}")

    st.markdown("---")

    # GRAPHIQUE DOUBLE : Histogramme + Boxplot
    c1, c2 = st.columns([1.4, 1])

    with c1:
        st.subheader(f"📈 Distribution comparative – {SENSOR_VARS[var_target]}")
        ...
        fig_hist.add_vline(x=stats_ok['mean'], line_dash='dash', line_color=COLOR_OK,
                           annotation_text=f"Moy. Sain", annotation_position="top left")
        fig_hist.add_vline(x=stats_ko['mean'], line_dash='dash', line_color=COLOR_FAIL,
                           annotation_text=f"Moy. Panne", annotation_position="top right")
    ...
    with c2:
        ...
        fig_box.update_layout(
            showlegend=False, xaxis_title='', yaxis_title=SENSOR_VARS[var_target],
            margin=dict(t=40, b=20)
        )
        st.plotly_chart(fig_box, use_container_width=True)

    # Indice de discrimination (Cohen's d simplifié)
    ...
    st.markdown(
        ...
        f"➡️ Une valeur > 0.8 signifie que cette variable seule peut détecter une panne avec une bonne fiabilité.</div>",
        unsafe_allow_html=True
    )

    # Distribution par type de machine
    st.markdown("---")
    st.subheader(f"🔍 Distribution de {SENSOR_VARS[var_target]} par type de machine")
    ...
    fig_violin.update_layout(
        xaxis_title='Type de machine (L=Low, M=Medium, H=High)',
